#ifndef LAB3_AES_AES_H
#define LAB3_AES_AES_H

#include <array>
#include <vector>

using word = std::array<unsigned char, 4>;

word operator^(const word &a, const word &b);

class AES {
public:
    explicit AES(std::string key);

    std::string encode(std::string data);
    std::string decode(std::string data);

private:
    static const constexpr int Nb = 4;
    static const constexpr int Nk = 4;
    static const constexpr int Nr = 10;

    static const constexpr int round_key_len = Nb * (Nr + 1);

    std::array<unsigned char, 4 * Nk> key;
    std::array<word, round_key_len> round_keys;

    std::array<unsigned char, 16> encode_block(std::array<unsigned char, 16> block);
    std::array<unsigned char, 16> decode_block(std::array<unsigned char, 16> block);

    void key_expansion();
    word sub_word(const word &w);
    word shift_row(const word &w, int shift);
    word inv_shift_row(const word &w, int shift);
    void add_round_key(std::array<unsigned char, 16> &state, int round_key_number);
    void sub_bytes(std::array<unsigned char, 16> &state);
    void inv_sub_bytes(std::array<unsigned char, 16> &state);
    void shift_rows(std::array<unsigned char, 16> &state);
    void inv_shift_rows(std::array<unsigned char, 16> &state);
    void mix_columns(std::array<unsigned char, 16> &state);
    void inv_mix_columns(std::array<unsigned char, 16> &state);
    unsigned char g_mul(unsigned char a, unsigned char b);


    std::vector<unsigned char> s_box = {
        
    };

    std::vector<unsigned char> inv_s_box = {
        
    };

    std::vector<word> r_con = {
        word{0x00, 0x00, 0x00, 0x00},
        word{0x01, 0x00, 0x00, 0x00},
        word{0x02, 0x00, 0x00, 0x00},
        word{0x04, 0x00, 0x00, 0x00},
        word{0x08, 0x00, 0x00, 0x00},
        word{0x10, 0x00, 0x00, 0x00},
        word{0x20, 0x00, 0x00, 0x00},
        word{0x40, 0x00, 0x00, 0x00},
        word{0x80, 0x00, 0x00, 0x00},
        word{0x1b, 0x00, 0x00, 0x00},
        word{0x36, 0x00, 0x00, 0x00}
    };
};

#endif //LAB3_AES_AES_H
